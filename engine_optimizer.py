...
DATA_FILE = 'data_raw.json'           # ต้องตรงกับ RAW_DATA_FILE ใน Scraper
OUTPUT_SETTINGS = 'config_markets.json' 
BACKTEST_DAYS = 90
PAYOUT = 100
                ...
                for k in range(1, BACKTEST_DAYS + 1):
                    ...
                    for b in BOTS_LIST:
                        ...
                        for i in range(1, lookback + 1):
                            idx = k + i
                            if idx >= len(draws) or not matrix[b['id']].get(idx): continue
                            if matrix[b['id']][idx][0] in draws[idx-1]['twoTop']: hits += 1
                    ...
                    sig = 'YELLOW' if (chaos > base and top > sec * VETO_MULT) else ('RED' if chaos > base else 'GREEN')
                    if sig != 'RED':
                        ...
                        if HARDCORE_SPLIT_MODE and sig == 'YELLOW' and TARGET_DIGITS >= 2:
                            ...
                            win_m = (PAYOUT * 1.0) if played[0] in draws[k]['twoTop'] else ((PAYOUT * YELLOW_BET_RATE) if played[1] in draws[k]['twoTop'] else 0)
                            sim_profit += win_m - cost
                        else:
                            rate = 1.0 if sig == 'GREEN' else YELLOW_BET_RATE
                            sim_profit += (PAYOUT * rate - COST_UNIT * rate) if any(d in draws[k]['twoTop'] for d in played) else -COST_UNIT * rate
